...
static void BuildIndices(const char *feedsFileName,hashset *all, hashset *stops,hashset *Url);
static void ProcessFeed(const char *remoteDocumentName,hashset *all, hashset *stops,hashset *Url);
static void PullAllNewsItems(FILE *dataStream,hashset *all, hashset *stops,hashset *Url);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st,hashset *all, hashset *stops,hashset *Url);
...
static void ParseArticle(const char *articleTitle,
                         ...
                         const char *articleURL,hashset *all, hashset *stops,hashset *Url);
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
                        const char *unused, const char *articleURL,hashset *all, hashset *stops,hashset *Url);
static void QueryIndices(hashset *all, hashset *stops);
static void ProcessResponse(const char *word, hashset *all, hashset *stops);
...
static const char *const kstopTextFileName = "data/stop-words.txt";
...
static int nodeHash(const void *str, int numBuckets)  
{            
  int i;
  unsigned long hashcode = 0;
  char * s = ( *(node**)str ) -> word;
...
void fillStops(hashset * stops) {
  FILE *infile;
  streamtokenizer st;
  char buffer[1024];

  infile = fopen(kstopTextFileName, "r");
  assert(infile != NULL);
  STNew(&st, infile, kNewLineDelimiters, true);

  while (STNextToken(&st, buffer, sizeof(buffer))) {
    char *stop =strdup(buffer);
    HashSetEnter(stops,&stop);
  }
  STDispose(&st);
  fclose(infile);
}
void getFree(void *addr){
  char *str =*(char**)addr;
  free(str);
}
void getArticleFree(void * addr){
  free(((article*) addr)->title);
  free(((article*) addr)->url);
}
void getFreeNode(void * addr){
  free(( *(node**)addr ) -> word);
  VectorDispose(( *(node**)addr ) -> info);
  free(( *(node**)addr ) -> info);
  free( *(void**)addr);
}
int compareStr(const void* addr1, const void * addr2){
  char * first =*((char**)addr1);
  char * second =*((char**)addr2);
  return strcasecmp(first,second);
}
int compareArticle(const void* addr1, const void * addr2){
  article * first = (article*)addr1;
  article * second = (article*) addr2;
  return strcasecmp(first->url, second->url);
}
static int compareVectors(const void* addr1, const void * addr2){
  article * first = (article*)addr1;
  article * second = (article*) addr2;
  if(first->word == second->word) return 0;
  
  if(first->word > second->word) return -1;
  
  return 1;
}
int compareNodeStr(const void* addr1, const void * addr2){
  char * first =(*(node**)addr1) ->word;
  char * second =(*(node**)addr2)->word;
  return strcasecmp(first,second);
}
...
int main(int argc, char **argv) {
  setbuf(stdout, NULL);
  hashset stops;
  HashSetNew(&stops, sizeof(char*), 901, StringHash, compareStr, getFree);
  hashset Url;
  HashSetNew(&Url, sizeof(char*), 901, StringHash, compareStr, getFree);
  hashset all;
  HashSetNew(&all, sizeof(node*), 1009, nodeHash, compareNodeStr, getFreeNode);
  fillStops(&stops);
  curl_global_init(CURL_GLOBAL_DEFAULT);
  Welcome(kWelcomeTextFile);
  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1],&all, &stops, &Url);
  QueryIndices(&all, &stops);
  HashSetDispose(&stops);
  HashSetDispose(&Url);
  HashSetDispose(&all);
  curl_global_cleanup();
  return 0;
...
static void BuildIndices(const char *feedsFileName,hashset *all, hashset *stops,hashset *Url) {
  ...
  while (STSkipUntil(&st, ":") !=
    ...
    ProcessFeed(remoteFileName,all,stops,Url);
  }
...
static void ProcessFeedFromFile(char *fileName,hashset *all, hashset *stops,hashset *Url) {
  FILE *infile;
  streamtokenizer st;
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription,
              (const char *)fileName,all, stops, Url);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew
                  // doesn't open one..
  fclose(infile);
}
...
static void ProcessFeed(const char *remoteDocumentName,hashset *all, hashset *stops,hashset *Url) {
  ...
  if (!strncmp(kFilePrefix, remoteDocumentName, strlen(kFilePrefix))) {
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(kFilePrefix),all, stops, Url);
    return;
  }

  FILE *tmpFeed = FetchURL(remoteDocumentName, "tmp_feed");
  PullAllNewsItems(tmpFeed,all, stops, Url);
  fclose(tmpFeed);
}
...
static void PullAllNewsItems(FILE *dataStream,hashset *all, hashset *stops,hashset *Url) {
  ...
  while (GetNextItemTag(
    ...
    ProcessSingleNewsItem(&st,all, stops, Url);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st,hashset *all, hashset *stops,hashset *Url) {
  ...
  ParseArticle(articleTitle, articleDescription, articleURL,all, stops,Url);
}
...
static void ParseArticle(const char *articleTitle,
  ...
  ScanArticle(&st, articleTitle, articleDescription, articleURL,all, stops,Url);
  STDispose(&st);
  fclose(tmpDoc);
}
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word)) {
        char* curr=word;
        numWords++;
        if(HashSetLookup(stops, &curr)==NULL){
            node* currInfo=malloc(sizeof(node));
            currInfo->word=strdup(word);
            currInfo->info=malloc(sizeof(vector));
            VectorNew(currInfo->info, sizeof(article),getArticleFree,32);
            node* hash =HashSetLookup(all,&currInfo);
            article currArticle;
            currArticle.title=strdup(articleTitle);
            currArticle.url=strdup(articleURL);
            currArticle.word=1;
            if(hash==NULL){
              VectorAppend(currInfo->info, &currArticle);
              HashSetEnter(all,&currInfo);
            }else{
              hash=*(node**)hash;
              int ans=VectorSearch(hash->info,&currArticle,compareArticle,0,false);
              getFreeNode(&currInfo);
              if(ans==-1) VectorAppend(hash->info, &currArticle);
              else{
                article* ansArticle=(article*) VectorNth(hash->info,ans);
                ansArticle->word++;
                free(currArticle.title);
                free(currArticle.url);
              }
            }
        }
...
static void QueryIndices(hashset *all, hashset *stops) {
  ...
  while (true) {
    ...
    if (strcasecmp(response, "") == 0)
      break;
    ProcessResponse(response,all,stops);
  }
}
...
static void ProcessResponse(const char *word,hashset *all, hashset *stops) {
  if (WordIsWellFormed(word)) {
    ...
    if(HashSetLookup(stops, &curr)==NULL){
        ...
        currInfo->info=malloc(sizeof(vector));
         VectorNew(currInfo->info, sizeof(article), getArticleFree, 32);
        if(HashSetLookup(all, &currInfo)!=NULL){
        node* nd= malloc (sizeof(node));
        nd->word = strdup(word);
        nd->info = malloc(sizeof(vector));
        VectorNew(nd->info, sizeof(article), getArticleFree, 32);
        node* currNode = *(node**) HashSetLookup(all, &nd);
        getFreeNode(&nd);
        VectorSort(currNode->info, compareVectors);
        int x =  VectorLength(currNode->info);
        if(x>10) x=10;
        for(int i=0; i<x; i++){
          article * art = VectorNth(currNode->info, i);
          if(art->word == 1){
