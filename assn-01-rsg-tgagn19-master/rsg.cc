...
void random(map<string, Definition>& grammar, vector<string>& result, const string& s)
{
  
  if (grammar.find(s) == grammar.end()){
    result.push_back(s);
  }else {
    Production p = grammar[s].getRandomProduction();
    for (Production::iterator curr = p.begin(); curr != p.end(); ++curr)
      random(grammar, result, *curr);
  }
}
