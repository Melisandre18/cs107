...
def findOptimalAlignment(strand1, strand2, cache):
	...
	# one possible alignment, and of course it's optimal
        if (strand1,strand2) in cache:
            return cache[strand1,strand2]
	if len(strand1) == 0:
            cache[strand1,strand2]=len(strand2) * -2, len(strand2)*" ",len(strand2)*"2", len(strand2) * " ", strand2
            return len(strand2) * -2, len(strand2)*" ",len(strand2)*"2", len(strand2) * " ", strand2
        
	if len(strand2) == 0:
            cache[strand1,strand2]= len(strand1)* -2,len(strand1)*" ", len(strand1)*"2", strand1, len(strand1)*" "
	...
	# they actually match.
        ...
        if strand1[0] == strand2[0]: 
            cache[strand1,strand2]=bestWith[0]+1,"1"+bestWith[1]," "+bestWith[2], strand1[0]+bestWith[3],strand2[0]+bestWith[4]
...
def printAlignment(alignment, out = sys.stdout):
    ...
    out.write("Optimal alignment score is " + str(alignment[0]) + "\n")
...
def main():
	...
	while (True):
            ...
            if answer == "no\n": break
            strand1 = generateRandomDNAStrand(100, 120)
            strand2 = generateRandomDNAStrand(100, 120)
            ...
            cache[strand1,strand2]=alignment
