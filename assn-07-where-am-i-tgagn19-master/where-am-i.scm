...
(define (intersection-points pts)
  ...
  (if (< (length pts) 2)
    ...
    (append (intersections-with-first-point)
            (intersection-points (cdr pts)))))

(define (distance-product pts1 pts2)
  (let ((pts2-without-pts1 (remove pts1 pts2)))
    (apply * (map (lambda (pt2) (dist pts1 pt2)) pts2-without-pts1))))   

(define (rate-points pts)
  (map (lambda (pt) (list (distance-product pt pts) pt)) pts))


(define (sort-points pts)
  (quicksort pts (lambda (pt1 pt2) (< (car pt1) (car pt2)))))

(define (clumped-points pts)
  (let ((sorted-pts (map cadr (sort-points (rate-points pts)))))
    (prefix-of-list sorted-pts (floor (/ (length sorted-pts) 2)))))
    
(define (average ls)
  (/ (apply + ls) (length ls)))

(define (average-point pts)
  (let ((avg-pt (list (average (map car pts))
                      (average (map cadr pts)))))
    (list (distance-product avg-pt pts) avg-pt)))

(define (best-estimate bst)
  (average-point (clumped-points (intersection-points bst))))
